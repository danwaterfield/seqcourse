...
def test_traminer_reference_fixture_matches_core_outputs() -> None:
    ...
    for item in payload["datasets"].values():
        ...
        ham = distance_matrix(dataset, method="HAM")
        lcs_auto = distance_matrix(dataset, method="LCS", norm="auto")
        ...
        assert np.allclose(
            transition_rates(dataset),
            np.asarray(item["transition_rates"], dtype=float),
            atol=1e-6,
        )
        assert np.allclose(
            mean_time_in_state(dataset).to_numpy(dtype=float),
